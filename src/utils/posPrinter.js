 ...
 */
export function buildReceiptHTML({ items, orderNumber, date, subtotal, discount, total, paid, change, paymentMethod, clientName, companyName = "نقطة البيع", receiptNote = "" }) {
 ...
 */
export async function printPOSOrder({ cart, products, printers, orderNumber, date, discount, total, subtotal, paid, change, paymentMethod, clientName, companyName, receiptNote }) {
  ...
  if (mainPrinter || printers.length === 0) {
    const html = buildReceiptHTML({ items: cart, orderNumber, date, subtotal, discount, total, paid, change, paymentMethod, clientName, companyName, receiptNote });
    printHTML(html);
  }
}