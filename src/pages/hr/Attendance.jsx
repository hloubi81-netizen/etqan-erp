    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">تتبع حضور الموظفين</p>
        </div>
        <div className="flex items-center gap-2">
          <Input type="date" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} className="h-9 w-40" />
          <Button onClick={openAdd} className="gap-2"><Plus className="h-4 w-4" />تسجيل</Button>
        </div>
      </div>
        ...
        <div className="overflow-x-auto rounded-xl border border-border">
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr>{["الموظف","التاريخ","النوع","دخول","خروج","ساعات","إجراءات"].map((h) => <th key={h} className="p-3 text-right font-medium text-muted-foreground">{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? <tr><td colSpan={7} className="text-center text-muted-foreground py-10">لا توجد سجلات</td></tr> :
                filtered.map((r) => (
